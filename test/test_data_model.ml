...
struct
  ...
  let test_get_slice_discrete db =
    lwt ks = D.register_keyspace db "test_get_slice_discrete" in
      ...
      D.read_committed_transaction ks
        (fun tx ->
           get_slice tx T.tbl (DM.Keys ["a"; "c"]) DM.All_columns >|=
             aeq_slice
               (Some "c",
                ["a", "v", [ "k", "kk"; "v", "vv" ];
                 "c", "w", [ "k2", "kk2"; "w", "ww" ]]) >>
    ...
    let all = DM.All_columns in
      ...
      D.read_committed_transaction ks
        (fun tx ->
           ...
           get_slice tx T.tbl ~max_keys:1 (DM.Keys ["001"; "002"])
             (columns ["y"]) >|=
      ...
      D.read_committed_transaction ks
        (fun tx ->
           ...
           get_slice tx T.tbl ~max_keys:2 (DM.Keys ["003"; "001"; "002"]) all >|=
             aeq_slice (Some "002",
                        [ "001", "x", ["x", ""];
                          "002", "x", ["x", ""]]))
    ...
    lwt ks = D.register_keyspace db "test_get_slice_read_tx_data" in
      ...
      D.read_committed_transaction ks
        (fun tx ->
           ...
           get_slice tx T.tbl (key_range ()) DM.All_columns >|=
             aeq_slice
               (Some "b", ["a", "1", ["0", ""; "00", ""; "1", ""];
                           "b", "10", ["0", ""; "1", ""; "10", ""]]) >>
           get_slice tx T.tbl (key_range ()) (columns ["0"; "00"]) >|=
             aeq_slice ~msg:"key range"
               (Some "b", ["a", "00", ["0", ""; "00", ""];
                           "b", "0", ["0", ""]]) >>
           get_slice tx T.tbl (DM.Keys ["a"; "b"]) (columns ["0"; "00"]) >|=
  ...
  let test_get_slice_tricky_columns db =
    lwt ks = D.register_keyspace db "test_get_slice_tricky_columns" in
      ...
      D.read_committed_transaction ks
        (fun tx ->
           get_slice tx T.tbl (DM.Keys ["k"]) DM.All_columns >|=
             aeq_slice (Some "k", [ "k", "\0003", [ "\0003", "" ] ]) >>
           get_slice tx T.tbl (DM.Keys ["k\000"]) DM.All_columns >|=
      ...
      D.read_committed_transaction ks
        (fun tx ->
           ...
           let expect tx =
             ...
             expect tx (DM.Keys ["a"; "b"; "x"; "c"; "d"]) None
           in return expect)
    ...
    let add_data () =
      put_slice ks T.tbl
        [ "a", (List.init 10 (fun n -> (sprintf "%d" n, sprintf "a%d" n)));
          ...
          "c", ["1", "c1"] ] in
    let assertions tx =
      D.get_slice_values tx T.tbl (DM.Keys ["a"; "b"]) ["0"; "1"] >|=
        aeq_slice_columns
          (Some "b", ["a", [Some "a0"; Some "a1"];
                      "b", [Some "b0"; None]]) >>
      D.get_slice_values tx T.tbl (DM.Keys ["a"; "b"]) ["1"] >|=
        aeq_slice_columns
          (Some "b", ["a", [Some "a1"];
                      "b", [None]]) >>
  ...
  let test_delete_key db =
    ...
    let get_all tx = get_slice tx T.tbl (key_range ()) DM.All_columns in
      put_slice ks T.tbl
        [ "a", [ "x", ""; "y", ""; "z", "" ];
          "b", [ "x", ""; "y", ""; "z", "" ]] >>
      D.read_committed_transaction ks
        (fun tx ->
           ...
           in get_all tx >|= expect_after_del "with key range">>
              get_slice tx T.tbl (DM.Keys ["a"; "b"]) DM.All_columns >|=
    ...
    lwt ks = D.register_keyspace db "test_put_columns" in
      ...
      D.read_committed_transaction ks
        (fun tx ->
           ...
           get_slice tx T.tbl (DM.Keys ["c"]) DM.All_columns >|=
