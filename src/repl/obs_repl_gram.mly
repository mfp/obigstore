...
phrase : /* empty */  { Nothing }
  ...
  | TXID      { with_ks (fun keyspace ->
                           R.Get_transaction_id { R.Get_transaction_id.keyspace }) }
  | WATCH table id_list
              { with_ks (fun keyspace ->
                           R.Watch_keys { R.Watch_keys.keyspace;
                                          table = $2; keys = $3 }) }
  | WATCH table id id_list
              { with_ks
                  (fun keyspace ->
                     R.Watch_columns { R.Watch_columns.keyspace; table = $2;
                                       columns = [$3, $4] }) }
